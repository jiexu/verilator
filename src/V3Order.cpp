...
private:
    ...
    int  m_rightClkWidth; // the clk width on the RHS
	...
	if (AstVarRef* varrefp = nodep->rhsp()->castVarRef()) {
	    ...
	    m_inAss = true;
	    nodep->rhsp()->iterateAndNext(*this, (AstNUser*)&m_rightClkWidth);
	    m_inAss = false;
	}
    ...
    virtual void visit(AstVarRef* nodep, AstNUser* wp) {
	...
	if (m_inAss) {
	    int  lw = 0, rw = 0;
	    nodep->lhsp()->iterateAndNext(*this, (AstNUser*)&lw);
	    nodep->rhsp()->iterateAndNext(*this, (AstNUser*)&rw);
	    *((int*)wp) = lw + rw;
	}
    }
	...
	if (m_inAss) {
	    int w = 0;
	    nodep->iterateChildren(*this, (AstNUser*)&w);
	    if (w > nodep->width()) *((int*)wp) = nodep->width();
	    else *((int*)wp) = w;
	}
    }

    virtual void visit(AstSel* nodep, AstNUser* wp) {
	if (m_inAss) {
	    int w = 0;
	    nodep->iterateChildren(*this, (AstNUser*)&w);
	    if (w > nodep->width()) *((int*)wp) = nodep->width();
    ...
    virtual void visit(AstReplicate* nodep, AstNUser* wp) {
	if (m_inAss) {
	    int w = 0;
	    nodep->iterateChildren(*this, (AstNUser*)&w);
	    if (nodep->rhsp()->castConst())
		*((int*)wp) = w * nodep->rhsp()->castConst()->toUInt();
	    else
		*((int*)wp) = nodep->width(); // can not check in this case.
	}
    }

    virtual void visit(AstActive* nodep, AstNUser* wp) {
	m_inClocked = nodep->hasClocked();
	nodep->iterateChildren(*this, wp);
	m_inClocked = false;
    }
    ...
    // Default
    //--------------------
    virtual void visit(AstNode* nodep, AstNUser* wp) {
	nodep->iterateChildren(*this, wp);
    }
